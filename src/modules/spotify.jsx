...
const spotifyGetLibraryList = (access_token) => {
  ...
  return new Promise((resolve, reject) => {
    let readSequential = function (offset) {
      ...
      if (offset >= 1) resolve(tracks);
      else {
        spotifyGetLibraryTracks(access_token)
          .then(function (tr) {
            if (!Object.keys(tr).includes("error")) {
              for (let i = 0; i < tr.items.length; i++) {
                spotifyGetLibraryTracksFeatures(
                  access_token,
                  tr.items[i].track.id
                ).then(function (tr_f) {
                  if (!("error" in tr_f)) {
                    tracks.push({
                      release_date: parseInt(
                        tr.items[i].track.album.release_date.substring(0, 4)
                      ),
...
const spotifyGetLibrary = (access_token) => {
  return new Promise((resolve, reject) => {
    spotifyGetLibraryList(access_token).then(function (tr) {
      if (Object.keys(tr).includes("error")) resolve({ error: tr.error });
      else {
        ...
        for (let i = 0; i < tr.length; i++) {
          ...
          for (let j = 0; j < fields.length; j++) {
            let st = fields[j];
            let val =
              st == "popularity" || st == "duration_ms" ? el[st] : el[st] * 100;
            ...
            for (let k = 0; k < fields_maxmin.length; k++) {
              ...
              if (stats[st][minmax].length === 0) stats[st][minmax].push(newm);
                ...
                let valueToCheck =
                  minmax === "min"
                    ? newm.value < stats[st][minmax][0].value
                    : newm.value > stats[st][minmax][0].value;
                if (valueToCheck) stats[st][minmax] = [newm];
                else if (newm.value === stats[st][minmax][0])
  ...
  return new Promise((resolve, reject) => {
    if (urlCompare() && localStorage.getItem("access_token") === null) {
      ...
      for (let i = 0; i < tokenArray.length; i++) {
        ...
        let value =
          key === "expires_in"
            ? parseInt(tokenDictValue[1]) + currentTime
            : tokenDictValue[1];
        localStorage.setItem(key, value);
      }
      if (statsAreEmpty)
        spotifyGetLibrary(localStorage.getItem("access_token")).then(
          (finalResult) => {
            resolve(finalResult);
          }
        );
